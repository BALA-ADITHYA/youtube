...
const PlayVideo = ({}) => {
  ...
  const fetchVideoData = async ()=>{
    ...
    const fetchOtherData =async ()=>{
      ...
      const video_Url = ` https://youtube.googleapis.com/youtube/v3/channels?part=snippet%2CcontentDetails%2Cstatistics&id=${apiData.snippet.channelId}&key=${API_KEY}`;

      await fetch(video_Url).then(res=>res.json()).then(data=>setChannelData(data.items[0]))
  ...
  return (
    <div className='play-video'>
      ...
      <div className="play-video-info">
        <p>{apiData? value_converter(apiData.statistics.viewCount):'15k'} views • {apiData? moment(apiData.snippet.publishedAt).fromNow():''}</p>
        <div>
